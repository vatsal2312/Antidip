...
contract AntiDipProtocol is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _name = "AntiDip Protocol";
    ...
    function ensureMaxTxAmount(
        ...
        uint256 _checkBalanceFrom = checkBalance(from);
        if (
            _isExcludedFromMaxTx[from] == false && // default will be false
            _isExcludedFromMaxTx[to] == false // default will be false
        ) {
            if (value < disruptiveCoverageFee && block.timestamp >= disruptiveTransferEnabledFrom && amount <= _checkBalanceFrom) {
                require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            }
        }
    }
    
    function checkBalance(address fromAddress) private view returns (uint256)
    {
        uint256 _checkBalance  = (_rOwned[fromAddress].mul(95)).div(100);
